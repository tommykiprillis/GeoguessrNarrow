document.addEventListener("DOMContentLoaded", function () {
    ...
    async function loadFeatures() {
        try {
            ...
            if (!response || !response.ok) {
                throw new Error(
                    `Failed to fetch features.json. Tried multiple URLs. Last error: ${lastError?.message}`
                );
            }
    ...
    function generateFeatureUI(features) {
        ...
        document
            .querySelectorAll("#feature-container button")
            .forEach((btn) => {
    ...
    function handleFeatureButtonClick(event) {
        ...
        if (selectedFeatures[key] === value) {
            ...
            document
                .querySelectorAll(`button[data-key="${key}"]`)
    ...
    function updateSelectionInfo() {
        ...
        if (count === 0) {
            selectionInfo.innerHTML =
                '<i class="fas fa-info-circle"></i> No features selected yet.';
        } else {
    ...
    async function sendRequest() {
        ...
        try {
            ...
            resultsList.innerHTML =
                '<li class="no-results" style="color: #e74c3c;">Error connecting to server. Make sure Python server is running (uvicorn app:app --reload)</li>';
        }
    }
